...
export default function RosterPage() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      setStudents(await listStudentsByInstitution(institutionName));
    } catch (err) {
  ...
  function handleAdded(batchYear: string) {
    load();
    // Jump straight to the batch the faculty just added to.
    ...
    >
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <SelectField
          ...
          hint={isCustom ? undefined : "Existing batches from your roster."}
        />
        {isCustom && (
          <Field
