...
export default function AdminStudentsUploadPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      const data = await uploadStudentsAsAdmin(excelFile, certificates, batchYear, institutionId);
      setResult(data);
      setExcelFile(null);
      setCertificates([]);
    } catch (err) {
    ...
    <div className="max-w-4xl">
      ...
      {!canCreate && (
        <div className="mt-6 max-w-xl">
          <NoPermissionNote action="add or upload students" />
        </div>
      )}
      ...
      <fieldset disabled={!canCreate} className={canCreate ? undefined : "pointer-events-none opacity-50"}>
      ...
      {mode === "single" && (
        <div className="mt-6">
          <AddSingleStudentForm
            ...
            batchYear={batchYear}
            disabled={!canCreate || institutions.length === 0}
            onSubmit={addStudentAsAdmin}
          />
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-6">
        ...
        <div>
          <Button type="submit" loading={submitting} disabled={institutions.length === 0 || !canCreate}>
            <UploadCloud size={16} />
            Upload roster
          </Button>
        </div>
